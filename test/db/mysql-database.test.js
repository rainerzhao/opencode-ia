...
test('uses a real MySQL 8.4 database with utf8mb4 UTC, ngram and repeatable versioned migrations', { skip: !testUrl }, async (t) => {
  ...
  const tables = await db.many(`
    ...
    'solutions', 'solution_versions', 'content_references', 'conversation_reference_details', 'content_attachments', 'business_units', 'requirements', 'requirement_interactions', 'schema_migrations'
    ) ORDER BY table_name
  `);
  assert.deepEqual(tables.map((row) => row.name), [
    ...
    'opencode_sessions', 'requirement_interactions', 'requirements', 'schema_migrations', 'skill_files', 'skill_installations',
