...
**IN PROGRESS** — Stage 2A–2C 通过验收；Stage 2D 的私人 Conversation API 与服务端可续传 WebSocket 已通过，生产聊天切换与 UI 尚未完成；Stage 2E 尚未开发。
...
- Test coverage：新行为经过 RED → GREEN；当前完整回归 178/178 通过，并包含 20 用户确定性模拟、WebSocket 断线补发、恢复边界和超大帧拒绝。
...
- Docs/migration：README、路线图和架构状态明确区分已完成的服务端协议与尚未完成的产品聊天切换。
...
- Stage 2D 多会话产品链路：Conversation API 与服务端实时协议通过；生产组合、React 界面和浏览器验收待完成。
- Stage 2E 恢复与总验收：未开始。
...
- 现有成员聊天仍使用每消息一次 `opencode run`；新 Gateway 协议只在注入 Gateway Service 时启用。
- 浏览器多会话、用户可见恢复边界和生产入口切换由 Stage 2D 后续子阶段验证。
