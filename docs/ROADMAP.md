...
| Stage 3 知识与方案 | 🚧 3A–3B 已完成 | FTS5 检索、版本、私有到发布流程 | 知识可查、可审、可撤回、可追溯 |
...
- ⏳ Stage 3C：对话沉淀为方案、方案转知识、引用展示与来源追溯。
- ⏳ Stage 3D：附件解析、导入导出、备份恢复、版本差异与完整验收。

Stage 3A–3B 让 SQLite 成为内容元数据、版本和检索的稳定真源：只索引当前知识版本，私人草稿不会因误设团队可见性而对成员暴露；新建知识和方案通过版本化 REST 与 React 界面保存，发布或撤回必须页面内二次确认并新增不可变版本。方案保留 Conversation、模型、知识版本和 Skill 版本等来源 ID，但默认列表不返回私人方案正文。旧文件接口仍在兼容期；来源转换、附件、导入导出与备份尚未实现，不能将 Stage 3 写成完成或生产就绪。
...
已决定以 MySQL 8.4 替代 SQLite：Mac 开发、测试与 Demo 使用本机 Docker MySQL 8.4，Linux 使用同主版本的内网 MySQL。数据库基础已在真实容器验证字符集、UTC、中文 `ngram`、迁移版本记录、并发迁移锁和来源引用去重；用户、登录 Session、审计 Repository 与认证 HTTP 链路也已完成真库验收。Gateway WebSocket 已适配异步身份校验，但 Gateway、Skill、知识和方案仓储仍待逐层异步切换。迁移未结束前，SQLite 的既有章节只描述历史实现，不能将 MySQL 或 Linux 写成已完成上线。
