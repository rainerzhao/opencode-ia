...
项目当前处于 **Stage 0、Stage 1、Stage 2、Stage 4 已完成 Mac 验收；Stage 3A 内容数据底座已完成，Stage 3B–3D 待开发** 的状态。
...
| Stage 3 知识与方案 | 🚧 3A 已完成 | FTS5 检索、版本、私有到发布流程 | 知识可查、可审、可撤回、可追溯 |
...
- ⏳ Stage 3B：旧文件接口兼容迁移、私有草稿编辑、人工发布与撤回。
- ⏳ Stage 3C：对话沉淀为方案、方案转知识、引用展示与来源追溯。
- ⏳ Stage 3D：附件解析、导入导出、备份恢复、版本差异与完整验收。

Stage 3A 让 SQLite 成为后续内容元数据、版本和检索的稳定真源：只索引当前知识版本，私人草稿不会因误设团队可见性而对成员暴露；方案保留 Conversation、模型、知识版本和 Skill 版本等来源 ID，但默认列表不返回私人方案正文。现有文件型知识和方案界面仍在迁移兼容期，尚未切换到该 Store；发布、撤回、来源转换与备份尚未实现，不能将 Stage 3 写成完成或生产就绪。
