...
项目当前处于 **Stage 0、Stage 1A–1D 完成，Stage 1E 待启动** 的状态。
...
| Stage 1 产品底座 | 🚧 1D 已完成 | React/Vite、账号密码、SQLite、角色与审计 | 多用户身份清晰、数据可追踪、默认私有 |
...
Stage 1D 已在 Mac 上跑通管理员与成员浏览器闭环；Stage 1E 仍需完成 React/Vite 迁移、资源本地打包和 Stage 1 总验收，因此仍不能作为内网生产系统开放。
