...
项目当前处于 **Stage 0、Stage 1 完成，Stage 2A 完成，Stage 2B 待开发** 的状态。

Mac 上已经跑通 React 前后端、账号权限、默认私有数据边界和无密钥 Demo。Stage 2A 已增加 Gateway 的持久数据模型、任务状态机和事件协议，但尚未接入常驻 OpenCode Worker，因此仍不代表公司内网生产可用。
...
| Stage 2 OpenCode Gateway | 🚧 2A 已完成 | 常驻 OpenCode 服务、会话映射、模型目录 | 不再每条消息冷启动，15–20 人会话可控 |
...
- ▶ Stage 2B：单个受保护的常驻 OpenCode Worker、HTTP/SSE 客户端和进程生命周期。
