...
MySQL 迁移 Phase A 已完成：Skill 异步仓储、校验/安装/启用服务和 HTTP 路由已接入同一异步契约，真实 MySQL 生命周期覆盖私人草稿隔离、人工发布、成员独立安装/启用和受控文件包。Phase B 已加入 MySQL-only 生产组合工厂和启动能力检查，并保留 SQLite Demo 组合；当前仅完成代码级装配与注入边界，真实 MySQL 全栈 HTTP/WebSocket 验收、知识/方案组合和 Linux 部署仍未完成。
