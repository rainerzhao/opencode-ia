...
function createMySqlGatewayStore(db, { idFactory = crypto.randomUUID, clock = () => new Date().toISOString() } = {}) {
  ...
  return Object.freeze({ appendEvent: (input) => db.transaction((tx) => insertEvent(tx, input)), bindOpenCodeSession, createConversation, createJob, getJob, getOpenCodeSession: async ({ conversationId }) => session(await db.one('SELECT * FROM opencode_sessions WHERE conversation_id = ?', [conversationId])), getOwnedConversation, listEventsAfter, recoverOnStartup, transitionJob: transition, upsertWorker });
}
