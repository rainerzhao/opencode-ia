...
**研发进展（2026-09-14）：** 已完成“需求与场景”后端底座及资产关联：管理员维护 BU；成员以默认私有方式保存需求和 IIM/电话/会议原始纪要，并关联多个自己的 Conversation、知识版本和方案版本。Conversation 完整历史支持受认证分页读取，不再受实时 WebSocket 回放窗口限制；关联只保存关系和版本，不复制正文、不扩大访问权限。该能力正在进入工作台页面开发，暂未纳入现有 Demo 的可视化入口。[查看当前目标与交付阶段](docs/PRODUCT_GOAL.md)。下方“当前可以体验”描述的是现有页面，不代表后续页面已经上线。
...
| 需求与场景业务底座 | ✅ P1A–P1B 后端已验证 | 管理员维护 BU；需求明确记录创建者和首任负责人；成员保存默认私有沟通并关联自己的会话、知识和方案版本；完整对话历史可分页读取。工作台页面尚未开发 |
