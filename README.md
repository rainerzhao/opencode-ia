...
**研发进展（2026-09-14）：** “需求与场景”已完成可用闭环：管理员可在账号管理内配置 BU 与团队统一字段；成员以默认私有方式创建、筛选、查看和编辑需求，记录 IIM/电话/会议原始纪要及受控字段值。成员可明确选择自己的对话事件范围，经现有 OpenCode Gateway 生成结构化草稿；草稿会在需求页回收状态，只有成员选择 BU、修改并确认后才会建立新的私有需求。需求还可关联自己仍有权限的 Conversation、知识和方案，关联不会改变原资产的可见性。字段定义有版本，管理员维护模板也不会获得成员私人正文或字段值。公司内部模型、云 MySQL 和 Linux 环境仍未验收。[查看当前目标与交付阶段](docs/PRODUCT_GOAL.md)。下方“当前可以体验”描述的是现有页面，不代表公司生产已经上线。
