...
**研发进展（2026-09-14）：** “需求与场景”已完成后端第一轮可追溯沉淀：管理员维护 BU 和团队统一字段；成员以默认私有方式保存需求、IIM/电话/会议原始纪要及受控字段值，并关联多个自己的 Conversation、知识版本和方案版本。字段定义有版本，旧需求保留当时的字段解释；管理员不因维护模板而获得成员私人正文或字段值。Conversation 完整历史支持受认证分页读取，不再受实时 WebSocket 回放窗口限制。P1C 的“经 OpenCode 对话生成草稿、人工确认保存”尚未实现；上述能力也尚未纳入现有 Demo 的可视化入口。[查看当前目标与交付阶段](docs/PRODUCT_GOAL.md)。下方“当前可以体验”描述的是现有页面，不代表后续页面已经上线。
...
| 需求与场景业务底座 | 🚧 P1A–P1C-A 后端已验证 | 管理员维护 BU 与统一字段；成员保存默认私有沟通、版本化字段值并关联自己的会话、知识和方案版本；完整对话历史可分页读取。OpenCode 草稿生成与工作台页面尚未开发 |
