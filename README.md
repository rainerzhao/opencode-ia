...
**研发进展（2026-09-14）：** “需求与场景”已完成第一版可用工作台：管理员可在账号管理内配置 BU 与团队统一字段；成员以默认私有方式创建、筛选、查看和编辑需求，记录 IIM/电话/会议原始纪要及受控字段值。字段定义有版本，旧需求保留当时的字段解释；管理员不因维护模板而获得成员私人正文或字段值。成员还可显式选择自己的对话范围，经现有 OpenCode Gateway 生成结构化需求草稿；AI 输出必须通过校验，且只有成员明确确认后才会创建新的私有需求。需求内 AI 草稿确认、关联会话/知识/方案的可视化流转仍在 P2B；公司内部模型和 Linux 环境也尚未验收。[查看当前目标与交付阶段](docs/PRODUCT_GOAL.md)。下方“当前可以体验”描述的是现有页面，不代表公司生产已经上线。
...
| 需求与场景工作台 | ✅ P1A–P2A 已完成 Demo 浏览器验收 | 管理员维护 BU 和统一字段；成员创建、筛选、编辑默认私有需求，记录 IIM/电话/会议原始材料和版本化字段值。需求内 AI 草稿、会话/知识/方案关联的可视化流转待 P2B |
