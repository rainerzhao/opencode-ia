...
**研发进展（2026-09-14）：** 已完成“需求与场景”业务后端底座：管理员可维护 BU 目录，成员的需求与 IIM/电话/会议原始纪要以默认私有方式保存、筛选和归档；管理员不因角色获得私人需求正文。该能力正在进入工作台页面开发，暂未纳入现有 Demo 的可视化入口。下一步围绕“我的工作台、需求与场景、方案资产、团队知识库”重构体验，把多个 AI 会话和相关资料关联到具体需求；个人聊天默认私有，有价值的内容经人工确认后再共享。[查看当前目标与交付阶段](docs/PRODUCT_GOAL.md)。下方“当前可以体验”描述的是现有页面，不代表后续页面已经上线。
...
| 需求与场景业务底座 | ✅ P1A 后端已验证 | 管理员维护 BU；需求明确记录创建者和首任负责人；成员保存默认私有的需求和原始沟通记录，支持筛选、分页、归档、审计及 MySQL 重启后的读取。工作台页面尚未开发 |
