...
> 当前版本用于产品体验和持续研发。Stage 2 常驻 Gateway 已在 Mac 完成多人多会话、运行管理、崩溃恢复及 OpenCode 标准工具面的应用级隔离验收；Stage 3A 已建立知识/方案的版本化 SQLite 与 FTS5 数据底座，发布闭环尚在研发；Stage 4A–4D 已完成默认私有草稿、校验、发布、按账号安装/启用、版本升级/回滚、停用/归档和真实 OpenCode 发现验证。Linux 进程级沙箱和生产部署仍在后续阶段。
...
| 知识与方案闭环 | 🚧 Stage 3A 已完成 | 已有版本化内容真源和当前版本 FTS5；人工发布、撤回、来源转换和备份仍在研发 |
