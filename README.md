...
| 运行管理 | 🚧 建设中 | 已提供管理员健康、任务查询和取消接口；可视化后台待接入，私人对话内容不向管理员展示 |
...
服务重启后可恢复尚未执行的排队任务，将结果未知的运行任务标记为中断，并检查原 OpenCode 会话是否仍可使用。无法恢复的上下文会记录恢复边界。管理员现可通过受保护接口查看健康、Worker 和任务状态并取消任务，不能读取私人标题或正文；可视化运维页面、运行期间 Worker 重启演练和真实内部模型验收仍待完成。
