...
> 当前版本用于产品体验和持续研发。多人私人对话、常驻 Runtime 多会话和运行管理已在 Mac 跑通；服务启动和 Runtime 故障后都会先验证会话上下文，再决定继续排队或要求成员确认。真实进程崩溃演练、工具执行隔离、团队 Skill 中心和 Linux 生产部署继续研发中。
...
多人产品的目标是：每人可以持续使用多个独立会话，由后台常驻运行服务统一承载；会话数量不等于进程数量。Mac 已通过 **1 个常驻 OpenCode Runtime、5 个账号、15 个会话、3 轮共 45 次真实模型请求**验收，峰值 15 个任务同时运行，跨轮方案标识及账号读取隔离检查通过。测试使用合成资料并禁用工具，尚不代表工具执行沙箱、Linux 容量或商业生产验收通过。
...
运行期间 Runtime 异常也会把关联会话置为恢复中：原 Session 可用时继续排队任务，不可用时中断相关任务并提示成员。当前已完成确定性故障测试，真实 OpenCode 进程崩溃演练仍是下一项验收。

- Stage 2E 已完成启动恢复、运行管理和真实多人多会话联调；运行期故障演练与工具执行隔离仍在研发。
